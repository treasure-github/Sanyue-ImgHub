        ...
        <el-container>
            ...
            <div class="header-content">
                ...
                <div class="search-card">
                    ...
                    <!-- <el-dropdown @command="showType" :hide-on-click="false" content="瀑布流格式" placement="bottom">
                        <span class="el-dropdown-link">
                            <font-awesome-icon icon="fa-th" class="header-icon" @click="handleChangeCss"></font-awesome-icon>
                        </span>
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item command="landscapeScreen">横屏</el-dropdown-item>
                                <el-dropdown-item command="verticalScreen">竖屏</el-dropdown-item>
                            </el-dropdown-menu>
                        </template>
                    </el-dropdown> -->
            ...
            <div :class="{content:!showStyle.flowerWater,waterfall:showStyle.flowerWater}" v-loading="loading">
                <template v-for="(item, index) in paginatedTableDataCache" :key="index">
                <el-card :class="{imgCard:!showStyle.flowerWater,autoCard:showStyle.flowerWater,elCardbody:showStyle.flowerWater}">

                    <el-checkbox v-model="item.selected" v-show="!showStyle.flowerWater"></el-checkbox>
                    ...
                    <el-image :class="{imagePreview:!showStyle.flowerWater}" v-else-if="isImage(item)" :preview-teleported="true" :src="'/file/' + item.name + '?from=admin'" :preview-src-list="item.previewSrcList" fit="cover" lazy></el-image>
                    <div v-else class="file-preview">
                        <font-awesome-icon icon="file" class="file-icon"></font-awesome-icon>
                    </div>
                    <div :class="{imageOverlay:!showStyle.flowerWater,imageOverlay2:showStyle.flowerWater}">
                    ...
                    <div :class="{fileInfo:!showStyle.flowerWater}" v-show="!showStyle.flowerWater">{{ item.metadata?.FileName || item.name }}</div>
                </el-card>
                </template>
            </div>
            <div class="pagination-container" v-show="!showStyle.flowerWater">
        ...
        <el-dialog title="文件详情" v-model="showdetailDialog" :width="dialogWidth">
            ...
            <el-descriptions direction="vertical" border :column="tableColumnNum">
                <el-descriptions-item 
                    ...
                    <el-image v-else-if="isImage(detailFile)" :src="'/file/' + detailFile?.name + '?from=admin'" fit="cover" lazy class="imagePreview"></el-image>
...
computed: {
    ...
    selectPage() {
        ...
        return this.paginatedTableDataCache.every(file => file.selected);
    },
    selectedPageFiles() {
        // 如果当前页有文件被选中，则返回 true，否则返回 false
        return this.paginatedTableDataCache.some(file => file.selected);
    },
    selectPageIcon() {
...
methods: {
    ...
    paginatedTableData() {
        ...
        this.paginatedTableDataCache = sortedData;
    },
    handleChangeCss(){
        if(this.showStyle.landscape || this.showStyle.vertical){
            this.showStyle.flowerWater = false
        }
    },
    handleDownload(key) {
    ...
    openDetailDialog(index, key) {
        this.detailFile = this.paginatedTableDataCache[index];
    ...
    handleCopy(index, key) {
        ...
        switch (this.defaultUrlFormat) {
            case 'originUrl':
                ...
                break;
            case 'mdUrl':
                text = `![${this.paginatedTableDataCache[index].metadata?.FileName || key}](${this.rootUrl}${key})`;
                break;
            case 'htmlUrl':
                text = `<img src="${this.rootUrl}${key}" alt="${this.paginatedTableDataCache[index].metadata?.FileName || key}" width=100%>`;
                break;
            case 'bbUrl':
                text = `[img]${this.rootUrl}${key}[/img]`;
                break;
            case 'tgId':
                text = this.paginatedTableDataCache[index].metadata?.TgFileId || 'none';
                break;
        }
    ...
    loadMoreData(showLoading=true) {
        ...
        // this.fetchWithAuth(`/api/manage/list?start=${start}&count=20`, { method: 'GET' })
        ...
        for (let index = 0; index < 15; index++) {
            this.tableData.push({
                "name": "pexels-josh-michaels-80899-28833005",
                "metadata": {
                    "FileName": "pexels-josh-michaels-80899-28833005.jpg",
                    "FileType": "image/jpeg",
                    "FileSize": "0.86",
                    "UploadIP": "8.210.60.87",
                    "ListType": "None",
                    "TimeStamp": 1740722159181,
                    "Label": "None",
        ...
        this.sortData(this.tableData);
        console.log('loadmoreData',this.tableData.length)
        this.loading = false;
    ...
    showType(command){
        this.showStyle.flowerWater = true
        this.showStyle.vertical = command=='verticalScreen';
        this.showStyle.landscape = command == 'landscapeScreen';
        if(this.showStyle.flowerWater){
    ...
    handleSelectPage() {
        if (this.selectPage) {
            this.paginatedTableDataCache.forEach(file => file.selected = false);
        } else {
            this.paginatedTableDataCache.forEach(file => file.selected = true);
        }
    },
    showFullScreen(){
...
mounted() {
    ...
    //             return this.fetchWithAuth("/api/manage/list?count=20", { method: 'GET' });
...
.el-card:hover .imageOverlay {
    opacity: 1;
}
.el-card:hover .imageOverlay2 {
    opacity: 1;
}
.overlay-buttons {
    display: flex;
